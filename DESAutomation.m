...
%Independent variables determined through regression
    ServiceTimeAct = 0.3423;           %Value derived from non-linear regression, 1/10th of a day = 48 min
    ReqServiceTime = 0.5994;           %Initial guess average amount of time a work item requires .08 of a day = 38.4 min
    ProjTaskArrivalRate = 0.2411;      %Initial guess project task interarrival rate coefficient
    MaintTaskArrivalRate = 0.1543;     %Initial guess maint task interarrival rate coefficient
    AdminTaskArrivalRate = 0.1713;     %Initial guess admin task interarrival rate coefficient
...
%    SE3UtilTT = timeseries2timetable(out.SE3Utilization);
    ...
    IncTimeTTData.Properties.VariableNames{(IncTimeTTColName)} = 'Data';
    ReqTimeTTData = ReqTimeTT;
    ReqTimeTTData.Properties.VariableNames{(ReqTimeTTColName)} = 'Data';
    ProjTimeTTData = ProjTimeTT;
    ProjTimeTTData.Properties.VariableNames{'TotalTimeProj'} = 'Data';
    MaintTimeTTData = MaintTimeTT;
    MaintTimeTTData.Properties.VariableNames{'TotalTimeMaint'} = 'Data';
    AdminTimeTTData = AdminTimeTT;
    AdminTimeTTData.Properties.VariableNames{'TotalTimeAdmin'} = 'Data';
    AllTimeTT = [IncTimeTTData; ReqTimeTTData; ProjTimeTTData; MaintTimeTTData; AdminTimeTTData];

    %Determine exponential distribution for all data and extract
    %coefficient (lambda)
    AllTimePDF = fitdist(AllTimeTT.Data,'Exponential');
    AllTimePDFlambda = 1/AllTimePDF.mu;
   ...
   %Analyze queue depth
    ...
    %Close the sddatain.xlsx file to prevent write error
        % Attach to an existing instance of Excel
        try
            excelApp = actxGetRunningServer('Excel.Application');
        catch
            error('No running instance of Excel found.');
        end
        
        % Specify the workbook name to close
        targetWorkbookName = 'sddatain.xlsx'; % The name of the workbook to search for
        
        % List all open workbooks
        disp('Listing all open workbooks...');
        for i = 1:excelApp.Workbooks.Count
            disp(['Workbook ', num2str(i), ': ', excelApp.Workbooks.Item(i).FullName]);
        end
        
        % Search for any workbook with the specified name and close it
        for i = excelApp.Workbooks.Count:-1:1 % Iterate backwards to avoid indexing issues when closing
            if strcmpi(excelApp.Workbooks.Item(i).Name, targetWorkbookName)
                disp(['Closing workbook: ', excelApp.Workbooks.Item(i).FullName]);
                % Close the workbook (use false to discard changes)
                excelApp.Workbooks.Item(i).Close(false); 
            end
        end
        
        % Release the COM object (optional, if no further operations needed)
        release(excelApp);
        disp('All matching workbooks closed.');

    %Output to Vensim
    SDParameters = cat(1,TicketsPickedUpPerDay,TasksPickedUpPerDay,TicketsStoppedPerDay,TasksStoppedPerDay,TicketsCompletedPerDay,TasksCompletedPerDay,BaseReworkRate,BaseIncFromChange,BaseMgmtPress,BaseFatigue,BaseQueuedTasks,BaseQueuedTickets,BaseMgmtPreempt,BaseCompleteTasks,BaseCompleteTickets,Iteration);
    writematrix(SDParameters, "C:\Users\enos9\OneDrive - Colostate\combined\sddatain.xlsx",'Sheet',1);

    %Record DES iteration data
    DESDataLabels = ["TicketsPickedUpPerDay";"TasksPickedUpPerDay";"TicketsStoppedPerDay";"TasksStoppedPerDay";"TicketsCompletedPerDay";"TasksCompletedPerDay";"BaseReworkRate";"BaseIncFromChange";"BaseMgmtPress";"BaseFatigue";"BaseQueuedTasks";"BaseQueuedTickets";"BaseMgmtPreempt";"BaseCompleteTasks";"BaseCompleteTickets";"Iteration"];
    if Iteration == 1
        DESOutputHistory = cat(2,DESDataLabels,SDParameters);
    else
        DESOutputHistory = cat(2,DESOutputHistory,SDParameters);
    end

%Trigger Vensim simulation, uses sddatain.xlsx and generates sddataout.xlsx

    %!"C:\Program Files\Vensim\vendss64.exe" "C:\Users\enos9\OneDrive -
    %Colostate\combined\scripts\sdcommand.cmd" is the old command
    ...
    % Use the start command to launch the process asynchronously
    ...
    % Monitor execution
    disp('Monitoring the external program...');
    startTime = tic;
    
    while true
        % Check if the process is running
        [~, result] = system('tasklist');
        if ~contains(result, 'taskkill /IM vendss64.exe') % Program no longer running
            disp('External program completed successfully.');
            break;
        end
        ...
        if elapsedTime > timeoutDuration
            disp('Timeout exceeded. Terminating external program...');
        ...
        % Pause briefly to avoid overloading the system with checks
        pause(1);
        
    end

%Post-process the Vensim output
SDDataOut = readmatrix("C:\Users\enos9\OneDrive - Colostate\combined\sddataout.xlsx",'Sheet',1); %extract data from Excel file
...
%Find Error Rate
    ...
    SDDataOutValMean = mean(SDDataOutVal,2); %average of each row across 5 days
    SDReworkRateMean = SDDataOutValMean(7,1); %average rework rate
    SDIncFrChgMean = SDDataOutValMean(6,1); %average rate of incidents from change
    SDErrorRateMean = SDReworkRateMean+SDIncFrChgMean; %Average combined errors / day
    SDErrorsOnRun = SDErrorRateMean*5; %Total errors during 5 day run
    SDCompTasksMean = SDDataOutValMean(1,1);
    if SDCompTasksMean < 0
        SDCompTasksMean = 0;
    end
    SDCompTicketsMean = SDDataOutValMean(2,1);
    SDAvgReworkPercentage = SDReworkRateMean / SDCompTasksMean;
    SDAvgIncFromChangePercentage = SDIncFrChgMean / SDCompTicketsMean;
    ErrorRate = (SDReworkRateMean + SDIncFrChgMean) / (SDCompTasksMean + SDCompTicketsMean); %Average error rate during run


%Find total service Time
    SDTasksCompleted = SDDataOut(2,7);
    SDTicketsCompleted = SDDataOut(3,7);
    SDTotalWorkCompleted = SDTasksCompleted+SDTicketsCompleted;
    SDAvgCompPerDay = SDTotalWorkCompleted / 5;
    SDServiceTimeActNew = 1 / SDAvgCompPerDay;  %THIS IS NOT THE RIGHT VALUE FOR DES Service Time - this is total elapsed time

%Find change in service time and calculate next iteration service time
    SDMgmtPreemptEnd = SDDataOut(5,7);
    SDMgmtChange = SDMgmtPreemptEnd-BaseMgmtPreempt;
    SDSvcTimeChange = (1+SDMgmtChange);
    ServiceTimeAct = ServiceTimeAct*SDSvcTimeChange;

%Record SD iteration data
    SDDataLabels = ["Time";"Completed Tasks";"Completed Tickets";"Fatigue";"Management Preemption";"Management Pressure";"Rate Inc from Change";"Rework Rate";"Task Completion Rate";"Task Pickup Rate";"Task Stop Rate";"Ticket Completion Rate";"Ticket Pickup Rate";"Ticket Stop Rate";"Time Required to Complete";"Total Queued Tasks";"Total Queued Tickets"];
    if Iteration == 1
        SDOutputHistory = cat(2,SDDataLabels,SDDataOutValWithTime);
    else
        SDOutputHistory = cat(2,SDOutputHistory,SDDataOutValWithTime);
    end

%Record DES next iteration data
    IterationNew = Iteration+1;
    DESInputHistoryNew = cat(1,ErrorRate,ServiceTimeAct,IterationNew);
    DESInputHistory = cat(2,DESInputHistory,DESInputHistoryNew);

%Extract carry-over variables
    BaseMgmtPreempt = SDDataOut(5,7);
    BaseMgmtPress = SDDataOut(6,7);
    BaseFatigue = SDDataOut(4,7)*.75; %Fatigue reduces somewhat over a weekend (between iterations)
    BaseQueuedTasks = SDDataOut(16,7);
    BaseQueuedTickets = SDDataOut(17,7);
    BaseCompleteTickets = SDTicketsCompleted;
    BaseCompleteTasks = SDTasksCompleted;
